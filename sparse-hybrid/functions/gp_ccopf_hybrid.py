...
class GP_CCOPF:
    def __init__(self, gp=None, gp_method=None, predefine_probability_y=None, predefine_probability_u=None,
                 ...
                 X=None, Y=None, Xd=None, alpha=None, per_load=None, per_rs=None, n_load=None, A_coef=None, b_inter=None, dc_u=None,
                 system='IEEE9'):
        ...
        # DC  
        dc_u = dc_u
        if self.__system == 'IEEE9':
            dc_u_cov = np.zeros((9,9))
            dc_u_sigma_cov = np.zeros((6,9))
        elif self.__system == 'IEEE39':
            dc_u_cov = np.zeros((39,39))
            dc_u_sigma_cov = np.zeros((35,39))
        ...
        dc_powe_flow_ = A_coef @ x_t + b_inter.reshape(-1,1)
        dc_powe_flow = ca.vertcat(dc_u, dc_powe_flow_)
        
        dc_sigma_ = A_coef @ cov_t @ A_coef.T
        dc_sigma = ca.blockcat(dc_u_cov, dc_u_sigma_cov.T, dc_u_sigma_cov, dc_sigma_)
        var_dc = ca.diag(dc_sigma)
        
        
        mean_pred = dc_powe_flow + mean_next_pred
        var_pred = var_dc + var_next_pred

        
        # outputs    
        opti.subject_to([mean-mean_pred==0, var-var_pred==0, var>=0])
        # alpha
    ...
    def input_covar(selsf, x, alpha, per_load, per_rs, n_u, n_load, n_rs):
        # matrix
        ...
        for i in range(n_u):
            for j in range(n_u):
                if i<j or i>j:
                    K_control[i,j] = (K_control[i,i] + K_control[j,j]) / 2
