...
def gp(X, X_new_mean, mu_m, A_m, invK, hyper, alpha=None):
    """ Gaussian Process
    ...
    for output in range(Ny):
        ...
        ks = np.zeros((N, 1))
        for i in range(N):
            ks[i] = cov_kernel(Xs[i, :].reshape(1,-1), X_new_mean, ell, sf2)
        kss = cov_kernel(X_new_mean, X_new_mean, ell, sf2)


        mean[output] = ks.T @ invK[output, :, :] @ mu_m[:, output]    
        var[output] = kss - ks.T @ invK[output, :, :] @ ks + ks.T @ invK[output, :, :] @ A_m[output, :, :] @ invK[output, :, :] @ ks

        
    return mean, var


def gp_ME(X, inputmean, mu_m, A_m, invK, hyper, alpha, chol):
    ...
    for output in range(Ny):
        ...
        ks = ca.MX.zeros(N, 1)
        for i in range(N):
            ks[i] = cov_se_ard(Xs[i, :].T, inputmean, ell, sf2)
...
def gp_TA1(X, Y, inputmean, inputcovar, mu_m, A_m, hyper, invK, alpha):
    ...
    for output in range(Ny):
        ...
        Xs = X[output, :, :]
        Xs  = ca.MX(Xs)
        v = Xs - ca.repmat(inputmean.T, N, 1)
...
def gp_TA2(X, Y, inputmean, inputcovar, mu_m, A_m, hyper, invK, alpha):
