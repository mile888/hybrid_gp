...
class GPR:
    def __init__(self, X, Y, M, Nc, optimizer_opts=None, normalize=True, multistart=1, hyper=None, meta=None):
        """ Optimize GPR model.
        ...
        if hyper is None:
            self.optimize(X=X, Y=Y, M=M, NoCandidates=Nc, opts=optimizer_opts, multistart=multistart, normalize=normalize)
        else:
            ...
            self.__Xs   = np.array(hyper['Xs'])
            self.__Ys   = np.array(hyper['Ys'])
            self.__mu_m  = np.array(hyper['mu_m'])
            self.__A_m  = np.array(hyper['A_m'])
    ...
    def optimize(self, X=None, Y=None, M=None, NoCandidates=None, opts=None, multistart=1, normalize=True, 
                 warm_start=False):
        ...
        opt = train_gp(self.__X, self.__Y, self.__M, self.__Nc, optimizer_opts=opts, multistart=multistart, hyper_init=hyp_init)
        ...
        self.__Xs  = opt['Xs']
        self.__Ys  = opt['Ys']
        self.__mu_m  = opt['mu_m_all']
        self.__A_m  = opt['A_m_all']
    ...
    def validate(self, X_test, Y_test, case='IEEE9'):
        ...
        for i in range(N):
            mean, var = gp(self.__Xs, X_test[i,:].reshape(1,-1), self.__mu_m, self.__A_m,
            ...
            if case == 'IEEE9':
                mean_pu = np.zeros((15,1))
                y_test_pu = np.zeros((15,1))

                mean_pu[:6] = mean[:6]/345
                mean_pu[6:] = mean[6:]/100
                y_test_pu[:6] = y_test[:6]/345
                y_test_pu[6:] = y_test[6:]/100
            elif case == 'IEEE39':
                mean_pu = np.zeros((74,1))
                y_test_pu = np.zeros((74,1))

                mean_pu[:29] = mean[:29]/345
                mean_pu[29:] = mean[29:]/400
                y_test_pu[:29] = y_test[:29]/345
                y_test_pu[29:] = y_test[29:]/400

                
            loss += (y_test_pu - mean_pu)**2
    ...
    def gp_ME(self, X_test_mean):
        mean, var = gp_ME(self.__Xs, X_test_mean, self.__mu_m, self.__A_m,
                                         self.__invK, self.__hyper, self.__alpha, self.__chol)
        return mean, var
    
    def gp_TA1(self, X_test_mean, X_test_covar):
        mean, var = gp_TA1(self.__Xs, self.__Ys, X_test_mean, X_test_covar, self.__mu_m, self.__A_m,
                                         self.__hyper, self.__invK, self.__alpha)
        return mean, var
    
    def gp_TA2(self, X_test_mean, X_test_covar):
        mean, var = gp_TA2(self.__Xs, self.__Ys, X_test_mean, X_test_covar, self.__mu_m, self.__A_m,
                                         self.__hyper, self.__invK, self.__alpha)
        return mean, var
    
    def gp_EM(self, X_test_mean, X_test_covar):
        mean, var = gp_EM(self.__Xs, self.__Ys, X_test_mean, X_test_covar,
                                      self.__hyper, self.__invK, self.__alpha)
        return mean, var
    ...
    def gp_(self, X_test_mean):
        mean, var = gp(self.__X, X_test_mean, self.__mu_m, self.__A_m,
                           self.__invK, self.__hyper, alpha=self.__alpha)
        return mean, var
    ...
    def to_dict(self):
        ...
        gp_dict['hyper'] = dict(
                    ...
                    Xs = self.__Xs.tolist(),
                    Ys = self.__Ys.tolist(),
                    mu_m  = self.__mu_m.tolist(),
                    A_m  = self.__A_m.tolist(),
